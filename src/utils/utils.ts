...
export function updateUrl(params) {
  const url2arr = location.href.split('?')
  if (url2arr[1]) {
    const queryArr = url2arr[1].split('&')
...
export function replaceUrl(params) {
  const newurl = updateUrl(params)
  window.history.replaceState(null, '', newurl) // 无刷新改变当前url
}
